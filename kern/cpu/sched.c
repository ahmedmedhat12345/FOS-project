...
void sched_init_PRIRR(uint8 numOfPriorities, uint8 quantum, uint32 starvThresh)
{
	...
	int kk=200;
	 stvTr =starvThresh ;
	 int c=100;
  for(int x=0;x<numOfPriorities;x++){
	  init_queue(&( ProcessQueues.env_ready_queues[x]));
  }
...
struct Env* fos_scheduler_PRIRR()
{
	...
	int c1=10;
		int z2=55;
	for (int x=0;x<num_of_ready_queues;x++){
        next_env = dequeue(&(ProcessQueues.env_ready_queues[x]));
		if (next_env != NULL){
			int aaa=10;
					int v=55;
             return next_env;
...
void clock_interrupt_handler(struct Trapframe* tf)
{
	...
	if (isSchedMethodPRIRR())
	{
		...
		//Comment the following line
		//panic("Not implemented yet");
                  if (timer_ticks() > stvTr){
                	  int kk=100;
		            for (int x=1;x<num_of_ready_queues;x++){
		            	acquire_spinlock(&(ProcessQueues.qlock));
		                LIST_FOREACH(proc,&(ProcessQueues.env_ready_queues[x])){
                              int kadad=100;
