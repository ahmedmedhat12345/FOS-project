...
int process_command(int number_of_arguments, char** arguments)
{
              LIST_INIT(&foundCommands);
int index = 0;
    while (index < NUM_OF_COMMANDS)
    {
                   if (strlen(arguments[0]) > strlen(commands[index].name))
        {
index++;
            continue;
        }
        ...
        char *ci = commands[index].name;
    char *initer = arguments[0];
for (; *initer && *ci; ci++)
            {
                        if (*initer == *ci)
                	initer++;
...
if (strcmp(arguments[0], commands[index].name) == 0)
        {
            LIST_INIT(&foundCommands);
  if ((commands[index].num_of_args == -1 && number_of_arguments > 1) ||
                  (number_of_arguments - 1 == commands[index].num_of_args))
            {
                return index;
            }
                           LIST_INSERT_TAIL(&foundCommands, &commands[index]);
