...
inline struct WorkingSetElement* env_page_ws_list_create_element(struct Env* e, uint32 virtual_address)
{
	//TODO: [PROJECT'24.MS2 - #07] [2] FAULT HANDLER I - Create a new WS element
		...
		//Your Code is Here...
		//uint32 va = strtol((uint32)virtual_address,NULL,16);
	                      uint32* table = NULL;
	struct WorkingSetElement* element = kmalloc(sizeof(struct WorkingSetElement));
		                      if (!element)
			panic(" failure ");
		element->virtual_address = ROUNDDOWN(virtual_address, PAGE_SIZE);
		                        get_frame_info(e->env_page_directory, element->virtual_address, &table)->element = element;
return element;
}
